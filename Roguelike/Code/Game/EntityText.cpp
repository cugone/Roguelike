...
void EntityText::EndFrame() {
    s_registry.erase(std::remove_if(std::begin(s_registry), std::end(s_registry), [](const auto& t)->bool { return !t || (t && MathUtils::IsEquivalentOrLessThanZero(t->GetStats().GetStat(StatsID::Health))); }), std::end(s_registry));
}
