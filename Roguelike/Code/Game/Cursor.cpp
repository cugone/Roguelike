...
void Cursor::AddVertsForCursor(Mesh::Builder& builder) const {
...
void Cursor::AddVertsForCursor(std::vector<Vertex3D>& vbo, std::vector<unsigned int>& ibo) const {
    const auto vert_left = _tile_coords.x + 0.0f;
    const auto vert_right = _tile_coords.x + 1.0f;
    const auto vert_top = _tile_coords.y + 0.0f;
    const auto vert_bottom = _tile_coords.y + 1.0f;
    ...
    const float z = 0.0f;
    const auto newColor = Rgba::White;
